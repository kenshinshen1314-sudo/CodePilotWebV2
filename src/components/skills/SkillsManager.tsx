...
export function SkillsManager() {
  ...
  const buildSkillUrl = useCallback((skill: SkillItem) => {
    ...
    // Pass source parameter to ensure we target the correct file
  ...
  const handleSave = useCallback(
    async (skill: SkillItem, content: string) => {
      // First verify the skill exists by fetching it
      const verifyUrl = buildSkillUrl(skill);
      const verifyRes = await fetch(verifyUrl);

      if (!verifyRes.ok) {
        // Skill doesn't exist, refresh the list and deselect
        await fetchSkills();
        setSelected(null);
        throw new Error("Skill file not found. The list has been refreshed.");
      }
  ...
  const handleDelete = useCallback(
    async (skill: SkillItem) => {
      ...
      if (res.ok) {
        ...
        // Refresh list to sync with actual file system state
        await fetchSkills();
      }
    },
    [buildSkillUrl, selected, fetchSkills]
  );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        {/* Left: skill list */}
        <div className="w-64 shrink-0 flex flex-col bg-muted/30 rounded-lg overflow-hidden">
          <div className="p-2">
