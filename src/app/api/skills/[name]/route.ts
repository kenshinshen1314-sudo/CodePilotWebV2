...
export async function GET(
  ...
  try {
    ...
    // Handle installed source (agents/claude)
    ...
    if (sourceParam && sourceParam !== "global" && sourceParam !== "project" && !installedSource) {
      return NextResponse.json(
        { error: "Invalid source; expected 'agents', 'claude', 'global', or 'project'" },
        { status: 400 }
      );
    }

    // Find skill based on source parameter
    let found;
    if (sourceParam === "global") {
      // Only search in global commands
      const globalPath = path.join(getGlobalCommandsDir(), `${name}.md`);
      if (fs.existsSync(globalPath)) {
        found = { filePath: globalPath, source: "global" as const };
      }
    } else if (sourceParam === "project") {
      // Only search in project (commands or skills)
      const projectPath = path.join(getProjectCommandsDir(cwdParam), `${name}.md`);
      if (fs.existsSync(projectPath)) {
        found = { filePath: projectPath, source: "project" as const };
      } else {
        const projectSkillPath = path.join(getProjectSkillsDir(cwdParam), name, "SKILL.md");
        if (fs.existsSync(projectSkillPath)) {
          found = { filePath: projectSkillPath, source: "project" as const };
        }
      }
    } else if (installedSource) {
      found = findSkillFile(name, { installedSource, installedOnly: true, cwd: cwdParam });
    } else {
      // No source specified, search all locations
      found = findSkillFile(name, { cwd: cwdParam });
    }
